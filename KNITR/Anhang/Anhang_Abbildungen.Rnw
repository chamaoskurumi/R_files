...
<<ResiduenEMAP, fig.scap="Karte der Residuen $\\hat{e}$ des SAR Modells \\textit{FortzügeB}", fig.cap="\\href{http://www.gmx.de}{Karte der Residuen $\\hat{e}$ des gewichteten SAR Modells auf \\textit{FortzügeB} (Modell 1)}", fig.height=4.8, fig.lp='fig:',fig.pos='H'>>=
ResiduenEMAP
@
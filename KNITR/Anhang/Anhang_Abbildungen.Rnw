...
Miete.2007MAP <- ggplot(LOR4reg.fort, aes(x=long, y=lat, group = id)) + 
  ...
  scale_fill_gradientn(colours = c(lowCOLOR,midCOLOR,hiCOLOR), 
                       ...
                       name=expression(paste('Miete'[2007],' ','(Euro)'))) +
...
Miete.2012MAP <- ggplot(LOR4reg.fort, aes(x=long, y=lat, group = id)) + 
  ...
  scale_fill_gradientn(colours = c(lowCOLOR,midCOLOR,hiCOLOR), 
                       values = rescale(c(4,7,14)),
                       guide = "colorbar", limits=c(4,14),
                       name=expression(paste('Miete'[2012],' ','(Euro)'))) +
  coord_map("polyconic",xlim = c(13.08,13.77),ylim = c(52.33,52.69)) + kartenlayout 
Miete.2012MAP
@
...
<<MosaicAssoc, fig.cap='Visualisierung des Zusammenhangs von $\\Delta_{r}Miete$ und $\\Delta Armut$ nach gewichteten Quartilen. Die Zelle Q4($\\Delta_{r}Miete$) / Q1($\\Delta Armut$) entspricht der \\textit{Kategorie} Gentri.', fig.subcap=c('Mosiakplot', 'Assoziationsplot'), out.width='0.48\\linewidth',fig.lp='fig:'>>=
...
\afterpage{
\begin{landscape}
<<ViolinFortZuzuegeI, fig.width=9.8, fig.height=5.6, fig.cap='Violinplots der wohnräumlichen Mobilität (\\textit{Fortzüge, Zuzüge, FortzügeA, ZuzügeA}) in der inneren Stadt',fig.lp='fig:'>>=

...
LOR4reg@data$GentriNEW <- -1
LOR4reg@data$GentriNEW[LOR4reg@data$ArmutchgQNTL=="4.Quartil" & LOR4reg@data$MietechgrQNTL=="4.Quartil"] <- "Gentri"

# Gewichtete deskriptive Statistiken von Miete.2007 nur für LORs der Kategorie "Gentri hi" 
Gentri_Miete.2007_DESCR <- describe(x=LOR4reg@data$Miete.2007[LOR4reg@data$GentriNEW=="Gentri"],
                                    weights=LOR4reg@data$E_E.2007[LOR4reg@data$GentriNEW=="Gentri"],
                                    exclude.missing=TRUE)
Gentri_Miete.2007_DESCR
# Zum Vergleich die ungewichteten deskriptiven Statistiken
describe(x=LOR4reg@data$Miete.2007[LOR4reg@data$GentriNEW=="Gentri"],
         #weights=LOR4reg@data$E_E.2007[LOR4reg@data$GentriNEW=="Gentri"],
         exclude.missing=TRUE)
...
Gentri_Armut.2007_DESCR <- describe(x=LOR4reg@data$Armut.2007[LOR4reg@data$GentriNEW=="Gentri"],
                                    weights=LOR4reg@data$E_E.2007[LOR4reg@data$GentriNEW=="Gentri"],
                                    exclude.missing=TRUE)
Gentri_Armut.2007_DESCR
# Zum Vergleich die ungewichteten deskriptiven Statistiken
describe(x=LOR4reg@data$Armut.2007[LOR4reg@data$GentriNEW=="Gentri"],
         #weights=LOR4reg@data$E_E.2007[LOR4reg@data$GentriNEW=="Gentri"],
         exclude.missing=TRUE)
...
LOR4reg@data$GentriNEW[LOR4reg@data$Armut.2007 > Gentri_Armut.2007_0.25 & 
                         LOR4reg@data$Armut.2007 < Gentri_Armut.2007_0.75 & 
                         LOR4reg@data$Miete.2007 > Gentri_Miete.2007_0.25 & 
                         LOR4reg@data$Miete.2007 < Gentri_Miete.2007_0.75 &
                         LOR4reg@data$GentriNEW!="Gentri"] <- "Control"
LOR4reg@data$GentriNEW[(LOR4reg@data$GentriNEW!="Gentri" & LOR4reg@data$GentriNEW!="Control")] <- "Other"
LOR4reg@data$GentriNEW[is.na(LOR4reg@data$MietechgrQNTL) |
                         is.na(LOR4reg@data$ArmutchgQNTL)  |
                         LOR4reg@data$valid=="ungültig"] <- NA
LOR4reg@data$GentriNEW <- as.factor(LOR4reg@data$GentriNEW)
table(LOR4reg@data$GentriNEW,useNA="ifany")
spplot(LOR4reg, zcol="GentriNEW", 
       col.regions=c("red","blue","grey"))
...
boxplot(LOR4reg@data$FortzuegeR ~ LOR4reg@data$GentriNEW)
boxplot(LOR4reg@data$ZuzuegeR ~ LOR4reg@data$GentriNEW)

by(LOR4reg@data$Miete.2007, LOR4reg@data$GentriNEW, summary)
by(LOR4reg@data$Armut.2007, LOR4reg@data$GentriNEW, summary)
by(LOR4reg@data$nicht_Alose_Hartz.2007, LOR4reg@data$GentriNEW, summary)
...
LOR4reg@data$GentriNEW[LOR4reg@data$Armut.2007 > Gentri_Armut.2007_0.05 & 
                         LOR4reg@data$Armut.2007 < Gentri_Armut.2007_0.95 & 
                         LOR4reg@data$Miete.2007 > Gentri_Miete.2007_0.05 & 
                         LOR4reg@data$Miete.2007 < Gentri_Miete.2007_0.95 &
                         LOR4reg@data$GentriNEW!="Gentri"] <- "Control"
LOR4reg@data$GentriNEW[(LOR4reg@data$GentriNEW!="Gentri" & LOR4reg@data$GentriNEW!="Control")] <- "Other"
...
### c.) ---- 0.10 < "Control" < 0.90 = ENDGÜLTIGE DEFINITION -----
...
LOR4reg@data$GentriNEW[LOR4reg@data$Armut.2007 > Gentri_Armut.2007_0.10 & 
...
boxplot(E_E.2012~interaction(GentriNEW,STADTRAUM), data=LOR4reg@data, col=3:1)
table(LOR4reg@data$E_E.2012,LOR4reg@data$GentriNEW,LOR4reg@data$STADTRAUM)
addmargins(table(LOR4reg@data$E_E.2012,LOR4reg@data$GentriNEW,LOR4reg@data$STADTRAUM), FUN = sum, quiet = FALSE)
...
bpDF <- subset(LOR4reg@data, !is.na(GentriNEW))

GENTRIplot <- function(data,var)
{
  localenv <- environment()
  p  <- ggplot(data, aes_string(x="GentriNEW", 
                                y=var, 
                                weight="E_E.2012",                         
                                fill="GentriNEW"),
               environment = localenv) + 
...
GentriDEFarmut0.1_0.9 <- wtd.quantile(x=bpDF$Armut.2007[bpDF$GentriNEW=="Gentri"], 
                                      weights=bpDF$E_E.2012[bpDF$GentriNEW=="Gentri"],             
                                      probs=c(0.1,0.9))
p  <- ggplot(bpDF, aes(GentriNEW, 
                       Armut.2007, 
                       weight=E_E.2012,
...
GentriDEFmiete0.1_0.9 <- wtd.quantile(x=bpDF$Miete.2007[bpDF$GentriNEW=="Gentri"], 
                                      weights=bpDF$E_E.2012[bpDF$GentriNEW=="Gentri"],             
                                      probs=c(0.1,0.9))
p  <- ggplot(bpDF, aes(GentriNEW, 
                       Miete.2007, 
...
by(LOR4reg@data$Armut.2007, LOR4reg@data$GentriNEW, summary)
by(LOR4reg@data$nicht_Alose_Hartz.2007, LOR4reg@data$GentriNEW, summary)
by(LOR4reg@data$E_65U110Rchg, LOR4reg@data$GentriNEW, summary)
...
maxval <- 17
dd <- bpDF %>% filter(FortzuegeR>maxval) %>%  group_by(GentriNEW) %>%  summarise(outlier_txt=paste(FortzuegeR,collapse=","))
p2 <- p1 + scale_y_continuous(limits=c(3,maxval)) +
  geom_text(data=dd,aes(y=maxval,label=outlier_txt),size=3,vjust=1.5,hjust=-0.5)
geom_segment(data=dd,aes(y=maxval*0.95,yend=maxval,
                         xend=factor(GentriNEW)),
             arrow = arrow(length = unit(0.1,"cm")))
p2
